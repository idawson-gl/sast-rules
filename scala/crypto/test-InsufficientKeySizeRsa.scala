...
class InsufficientKeySizeRsa {
  ...
  def weakKeySize2 = {
    val keyGen = KeyPairGenerator.getInstance("RSA")
    keyGen.initialize(128, new Nothing) //BAD //Different signature
  ...
  def weakKeySize3ParameterSpec = {
    val keyGen = KeyPairGenerator.getInstance("RSA")
    keyGen.initialize(new Nothing(128, RSAKeyGenParameterSpec.F4))
  ...
  def weakKeySize4ParameterSpec = {
    val keyGen = KeyPairGenerator.getInstance("RSA")
    keyGen.initialize(new Nothing(128, RSAKeyGenParameterSpec.F4), new Nothing)
    val key = keyGen.generateKeyPair
    key
  }
  ...
  def okKeySizeParameterSpec = {
    val keyGen = KeyPairGenerator.getInstance("RSA")
    keyGen.initialize(new Nothing(2048, RSAKeyGenParameterSpec.F4)) //Different signature
  ...
  def weakKeySizeWithProviderObject1 = {
    val keyGen = KeyPairGenerator.getInstance("RSA", new InsufficientKeySizeRsa#DummyProvider)
    keyGen.initialize(1024)
    keyGen.generateKeyPair
  }
  ...
  def weakKeySizeWithProviderObject2 = {
    val p = new InsufficientKeySizeRsa#DummyProvider("info")
  ...
  private class DummyProvider private[crypto](val info: Nothing) extends Nothing("dummy", 0.0, info) {
    def this {
      this("dummy")
    }
  }
