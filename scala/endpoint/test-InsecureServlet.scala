...
class InsecureServlet extends Nothing {
  @Override
  ...
  private def useParameters(req: Nothing): Unit = {
    val username = req.getParameter("username").asInstanceOf[Nothing]
    val roles = req.getParameterValues("roles").asInstanceOf[Array[Nothing]]
