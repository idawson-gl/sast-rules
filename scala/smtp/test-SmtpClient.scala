...
object SmtpClient {
  ...
  def sendEmail(input1: String, input2: String, input3: String, input4: String, input5: String, input6: String): Unit = {
    ...
    val session = Session.getInstance(props, new Nothing() {
      protected def getPasswordAuthentication = new Nothing(username, password)
    })
    val message = new MimeMessage(session)
    message.setFrom(new InternetAddress("source@gmail.com"))
    message.setRecipients(Message.RecipientType.TO, Array[InternetAddress](new InternetAddress("destination@gmail.com")))
